...
if (!empty($_POST)) {
    ...
    if (!isset($_POST['hoten'])) {
        ...
        $regexHoten = '/^([A-Z][a-zA-Z]*)(( {1}[A-Z][a-zA-Z]*){1,5})$/';
    ...
    if (!isset($_POST['so_dien_thoai'])) {
        $errors['so_dien_thoai'] = "Chưa nhập số điện thoại";
    } else {
        $regexSoDienThoai = '/^[0-9]{10,11}$/';
        $stringSoDienThoai = $_POST['so_dien_thoai'];
        if (!preg_match($regexSoDienThoai, $stringSoDienThoai)) {
            $errors['so_dien_thoai'] = "Số điện thoại không hợp lệ";
        }
    }

    if (!isset($_POST['sinh_nhat'])) {
        $errors['sinh_nhat'] = "Chưa nhập ngày sinh nhật";
    } else {
        $regexSinhNhat = '/^(0?[0-9]|[12][0-9]|3[01])(\/)(0?[1-9]|[1][012])(\/)([12][0-9][0-9][0-9])$/';
        $stringSinhNhat = $_POST['sinh_nhat'];
        if (!preg_match($regexSinhNhat, $stringSinhNhat)) {
            $errors['sinh_nhat'] = "Ngày sinh nhật không hợp lệ";
        }
    }

    if (!isset($_POST['ghi_chu'])) {
        $errors['ghi_chu'] = "Chưa nhập ghi chú";
    } else {
        $regexGhiChu = '/^(.+)(( {1}.*){7,199})$/';
        $stringGhiChu = $_POST['ghi_chu'];
        if (!preg_match($regexGhiChu, $stringGhiChu)) {
            $errors['sinh_nhat'] = "Ghi chú không hợp lệ";
        }
    }
